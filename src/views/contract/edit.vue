<template>
  <div class="app-bg">
      ...
      <div class="contract-field">
        ...
        <div>
          <van-row class="linkman"
                   v-if="fromData.construct_person">
            <van-col :span="12"
                     class="contract-field-title">
              联系人:{{fromData.construct_person}}
            </van-col>
            <van-col :span="12">
              联系电话:{{fromData.construct_personphone}}
            </van-col>
          </van-row>
        </div>
        ...
        <div>
          <van-row class="linkman"
                   v-if="fromData.build_person">
            <van-col :span="12"
                     class="contract-field-title">
              联系人:{{fromData.build_person}}
            </van-col>
            <van-col :span="12">
              联系电话:{{fromData.build_personphone}}
            </van-col>
          </van-row>
        </div>
        ...
        <div>
          <van-row class="linkman"
                   v-if="fromData.supervision_person">
            <van-col :span="12"
                     class="contract-field-title">
              联系人:{{fromData.supervision_person}}
            </van-col>
            <van-col :span="12">
              联系电话:{{fromData.supervision_personphone}}
            </van-col>
          </van-row>
        </div>
      </div>
    ...
    <div v-if="showLink=='3'"
         class="contract contract-add addField">
      <!-- <van-nav-bar left-text="编辑合同-下一步"
      ...
      <div class="contract-field">
        ...
        <van-row>
          <van-col :span="16"
                   :offset="4">
            <van-uploader class="z-large van-button van-button--primary van-button--large"
  ...
  data() {
    return {
      ...
      fromData: {
        ...
        construct_person: '', // 建设单位联系人
        construct_personphone: '', // 建设单位联系人电话
        build_unitid: '', // 承建单位
        build_unitname: '',
        build_personid: '', // 承建单位联系人ID
        build_person: '', // 承建单位联系人
        build_personphone: '', // 承建单位联系人电话
        supervision_unitid: '', // 监理单位
        supervision_unitname: '',
        supervision_personid: '', // 监理单位联系人ID
        supervision_person: '', // 监理单位联系人
        supervision_personphone: '', // 监理单位联系人电话
  ...
  methods: {
    ...
    async getCodeInfo() {
      ...
      if (!this.baseData.htglDuration) {
        let res = await getCodeInfoByCode('htglDuration');
        if (res.data.success) {
      ...
      if (!this.baseData.htglMoney) {
        let res = await getCodeInfoByCode('htglMoney');
        if (res.data.success) {
      ...
      if (!this.baseData.htglContractStatus) {
        // 合同状态
        let res = await getCodeInfoByCode('htglContractStatus');
        if (res.data.success) {
      ...
      if (!this.baseData.htglContractType) {
        // 合同类型
        let res = await getCodeInfoByCode('htglContractType');
      ...
      if (!this.baseData.htglPaymentMethod) {
        // 支付方式
        let res = await getCodeInfoByCode('htglPaymentMethod');
      ...
      if (!this.baseData.htglPaymentState) {
        // 支付计划名称
        let res = await getCodeInfoByCode('htglPaymentState');
        ...
        .then(res => {
          if (res.data.success) {
            for (let key in this.fromData) {
              this.fromData[key] = res.data.dmp[key];
            }
            this.fromData.contracttypeName = this.getBaseValueByCode(
              this.fromData.contracttype,
              this.baseData.htglContractType
            );
            this.fromData.monetaryName = this.getBaseValueByCode(
              this.fromData.monetary,
              this.baseData.htglMoney
            );
            this.fromData.constructionname = this.getBaseValueByCode(
              this.fromData.construction,
              this.baseData.htglDuration
            );
            this.fromData.statename = this.getBaseValueByCode(
              this.fromData.state,
              this.baseData.htglContractStatus
            );
            this.fromData.paymentname = this.getBaseValueByCode(
              this.fromData.payment,
              this.baseData.htglPaymentMethod
            );
            this.fromData.planList = res.data.planList;
            this.fromData.fileList = res.data.fileList;
            this.fileTotal = 0;
    ...
    setEndTime() {
      if (
        ...
        this.fromData.starttime == '请选择开始时间'
      ) {
        return false;
      }
      ...
      switch (this.fromData.construction) {
        case '1':
          this.fromData.endtime = this.Datemat.addDate(
            'day',
            this.fromData.starttime,
            this.fromData.period
          );
          break;
        case '2':
          this.fromData.endtime = this.Datemat.addDate(
    ...
    savelink(data) {
      ...
      switch (this.unitType) {
        case '1':
          this.fromData.construct_person = data.contact;
          this.fromData.construct_personphone = data.phone;
          ...
          break;
        case '2':
          this.fromData.build_person = data.contact;
          this.fromData.build_personphone = data.phone;
          ...
          break;
        case '3':
          this.fromData.supervision_person = data.contact;
          this.fromData.supervision_personphone = data.phone;
    ...
    savePlan(action, done) {
      if (action == 'confirm') {
        ...
        let row = {
          id: this.fromData.planList[this.editIndex]
            ? this.fromData.planList[this.editIndex].id
              ? this.fromData.planList[this.editIndex].id
              : ''
            : '',
          name: this.planname,
    ...
    saveContract() {
      ...
      editTContractInfo(this.fromData)
        .then(res => {
          if (res.data.success) {
            this.$router.push({ path: 'list' });
          } else {
